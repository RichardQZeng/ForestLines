...
from .geotrace.core.trace import leastCostPath, computeCostImage
...
class GeoTraceDockWidget(QtWidgets.QDockWidget, FORM_CLASS):
    ...
    def button_click(self, name):
        ...
        if name in ['cc_brightness', 'cc_darkness', 'cc_sobel', 'cc_prewitt', 'cc_laplace']:  # cost calculator
        ...
        elif name == 'tt_undo':  # undo operation
            pass
        elif name == 'ot_estimate':  # generate planar orientation estimates
            pass
        elif name == 'ot_impute':  # impute orientations
            pass
        elif name == 'mt_intensity':  # calculate intensity
            pass
        elif name == 'mt_thickness':  # calculate thickness
            pass

        widget = self.get_layerSelectBox("traces")
        log(widget.objectName())
